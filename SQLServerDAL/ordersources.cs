...
namespace Hangjing.SQLServerDAL
{
    ...
    public class ordersources 
    {
        ...
        /// 增加一条数据
        /// </summary>
        public int Add(ordersourcesInfo model)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("insert into ordersources(");
        ...
        /// 更新一条数据
        /// </summary>
        public int Update(ordersourcesInfo model)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update ordersources set ");
        ...
        /// 此代码由杭景科技代码内部生成器自动生成
        /// <param>ID</param>
        /// <returns>ordersourcesInfo</returns>
        public ordersourcesInfo GetModel(int ID)
        {
            string sql = "select ID,classname,Depth,Status,Priority,Parentid,isDel from ordersources where  ID = @ID";
            SqlParameter parameter = new SqlParameter("@ID", SqlDbType.Int, 4);
            parameter.Value = ID;
            ordersourcesInfo model = null;
            using (SqlDataReader dr = SQLHelper.ExecuteReader(CommandType.Text, sql, parameter))
            {
                if (dr.Read())
                {
                    model = new ordersourcesInfo();
        ...
        {
            string sql = "select classname from ordersources where  " + strwhere + "";
        ...
        ///此代码由杭景科技代码内部生成器自动生成
        public IList<ordersourcesInfo> GetList(int pagesize, int pageindex, string strWhere, string orderName, int orderType)
        {
            IList<ordersourcesInfo> infos = new List<ordersourcesInfo>();
            SqlParameter[] parameters = 
			{
				new SqlParameter("@tblName", SqlDbType.VarChar,255),
            ...
            parameters[0].Value = "ordersources";
            parameters[1].Value = "ID,classname,Depth,Status,Priority,Parentid,isDel";
            ...
            using (SqlDataReader dr = SQLHelper.ExecuteReader(CommandType.StoredProcedure, "pageselectpri", parameters))
            {
                while (dr.Read())
                {
                    ordersourcesInfo info = new ordersourcesInfo();
        ...
        public int DelShopData(int Id)
        {
            ...
            str.Append("delete from ordersources where ID=@ID");
            SqlParameter[] Para = 
			{
        ...
        {
            StringBuilder str = new StringBuilder();
            str.Append("delete from ordersources where ID in ({0})");
        ...
        /// <returns>图书列表</returns>
        ///此代码由杭景科技代码内部生成器自动生成
        public IList<ordersourcesInfo> GetsubList(int pid)
        {
            string sql = "select * from ordersources where parentid = @pid and isdel = 0 order by Priority desc";
            IList<ordersourcesInfo> infos = new List<ordersourcesInfo>();
            SqlParameter[] parameters = 
			{
