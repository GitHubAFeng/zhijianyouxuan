...
namespace Hangjing.SQLServerDAL
{
	...
	public class OrderDeliver
	{
		...
		/// 增加一条数据
		/// </summary>
		public int Add(OrderDeliverInfo model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into OrderDeliver(");
		...
		/// 更新一条数据
		/// </summary>
		public int Update(OrderDeliverInfo model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update OrderDeliver set ");
        ...
        /// 此代码由杭景科技代码内部生成器自动生成
        /// <param>DataId</param>
        /// <returns>OrderDeliverInfo</returns>
        public OrderDeliverInfo GetModel(int DataId)
        {
            string sql = "select DataId,OrderId,DeliverId,DeliverName,Dispatcher,DispatchTime,DeliveryTime,Section,Inve1,Inve2 from OrderDeliver where  DataId = @DataId";
            SqlParameter parameter = new SqlParameter("@DataId", SqlDbType.Int, 4);
            parameter.Value = DataId;
            OrderDeliverInfo model = null;
            using (SqlDataReader dr = SQLHelper.ExecuteReader(CommandType.Text, sql, parameter))
            {
                if (dr.Read())
                {
                    model = new OrderDeliverInfo();
        ...
        {
            return (int)SQLHelper.ExecuteScalar(CommandType.StoredProcedure, "pagecount", new SqlParameter("@tblName", "OrderDeliver"), new SqlParameter("@strWhere", strWhere));
        }
        ...
        ///此代码由杭景科技代码内部生成器自动生成
        public IList<OrderDeliverInfo> GetList(int pagesize, int pageindex, string strWhere, string orderName, int orderType)
        {
            IList<OrderDeliverInfo> infos = new List<OrderDeliverInfo>();
            SqlParameter[] parameters = 
			{
            ...
            parameters[0].Value = "OrderDeliver";
            parameters[1].Value = "*";
        ...
        public int DelOrderDeliver(int Id)
        {
            ...
            str.Append("delete from OrderDeliver where DataId=@DataId");
            SqlParameter[] Para = 
			{
        ...
        {
            StringBuilder str = new StringBuilder();
            str.Append("delete from OrderDeliver where DataId in ({0})");
        ...
        public int UpdataOrderDeliver(string OrderID, DateTime DeliveryTime, int Interval)
        {
            ...
            str.Append("update OrderDeliver set DeliveryTime=@DeliveryTime,Inve1=@Inve1 where OrderID=@OrderID");
        ...
        /// <param>DataId</param>
        /// <returns>OrderDeliverInfo</returns>
        public OrderDeliverInfo GetModel(string orderid)
        {
            string sql = "select DataId,OrderId,DeliverId,DeliverName,Dispatcher,DispatchTime,DeliveryTime,Section,Inve1,Inve2 from OrderDeliver where  OrderId = @DataId";
        ...
        {
            int AVGusesecond = 0;
            string sql = "SELECT avg(datediff(minute, DispatchTime,OverTime)) as usesecond  FROM OrderDeliver WHERE OverTime IS NOT  NULL and " + where;
