...
namespace Hangjing.SQLServerDAL
{
    ...
    public partial class OrderPromotion
    {
        ...
        /// 增加一条数据
        /// </summary>
        public int Add(OrderPromotionInfo model)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("insert into OrderPromotion(");
        ...
        /// 更新一条数据
        /// </summary>
        public int Update(OrderPromotionInfo model)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update OrderPromotion set ");
        ...
        /// 此代码由杭景科技代码内部生成器自动生成
        /// <param>pId</param>
        /// <returns>OrderPromotionInfo</returns>
        public OrderPromotionInfo GetModel(int pId)
        {
            string sql = "select pId,shopid,startdate,enddate,starttime,endtime,ptype,isopen,freeSendFee,overmoney,minusmoney,reveint1,reveint2,revevar1,revevar2,revefloat1,revefloat2 from OrderPromotion where  pId = @pId";
            SqlParameter parameter = new SqlParameter("@pId", SqlDbType.Int, 4);
            parameter.Value = pId;
            OrderPromotionInfo model = null;
            using (SqlDataReader dr = SQLHelper.ExecuteReader(CommandType.Text, sql, parameter))
            {
                if (dr.Read())
                {
                    model = new OrderPromotionInfo();
        ...
        {
            return (int)SQLHelper.ExecuteScalar(CommandType.StoredProcedure, "pagecount", new SqlParameter("@tblName", "OrderPromotion"), new SqlParameter("@strWhere", strWhere));
        }
        ...
        /// <returns>列表</returns>
        ///此代码由杭景科技代码内部生成器自动生成
        public IList<OrderPromotionInfo> GetList(int pagesize, int pageindex, string strWhere, string orderName, int orderType)
        {
            IList<OrderPromotionInfo> infos = new List<OrderPromotionInfo>();
            SqlParameter[] parameters =
            {
            ...
            parameters[0].Value = "OrderPromotion";
            parameters[1].Value = "*,(select name from points where unid = OrderPromotion.shopid ) as title ";
            ...
            using (SqlDataReader dr = SQLHelper.ExecuteReader(CommandType.StoredProcedure, "pageselectpri", parameters))
            {
                while (dr.Read())
                {
                    OrderPromotionInfo model = new OrderPromotionInfo();
                    model.pId = HJConvert.ToInt32(dr["pId"]);
                    model.shopid = HJConvert.ToInt32(dr["shopid"]);
                    model.startdate = HJConvert.ToDateTime(dr["startdate"]);
                    model.enddate = HJConvert.ToDateTime(dr["enddate"]);
        ...
        /// 此代码由杭景科技代码内部生成器自动生成
        /// <returns></returns>
        public int DelOrderPromotion(int Id)
        {
            StringBuilder str = new StringBuilder();
            str.Append("delete from OrderPromotion where pId=@pId");
            SqlParameter[] Para =
                {
        ...
        {
            StringBuilder str = new StringBuilder();
            str.Append("delete from OrderPromotion where pId in ({0})");
