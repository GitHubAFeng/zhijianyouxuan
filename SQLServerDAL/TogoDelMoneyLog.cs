...
{
    /// <summary>
    /// 数据访问类TogoDelMoneyLog。
    /// </summary>
    public class TogoDelMoneyLog
    {
        /// <summary>
        /// 增加一条数据
        /// </summary>
        public int Add(TogoDelMoneyLogInfo model)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("insert into TogoDelMoneyLog(");
        ...
        /// 更新一条数据
        /// </summary>
        public int Update(Hangjing.Model.TogoDelMoneyLogInfo model)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update TogoDelMoneyLog set ");
        ...
        /// 此代码由杭景科技代码内部生成器自动生成
        /// <param>DataId</param>
        /// <returns>TogoDelMoneyLogInfo</returns>
        public TogoDelMoneyLogInfo GetModel(int DataId)
        {
            string sql = "select DataId,UserId,DelMoney,AddDate,BuyItem,Inve1,Inve2,(select Name from points where unid = TogoDelMoneyLog.UserId)as TogoName from TogoDelMoneyLog where DataId=@DataId ";
            SqlParameter parameter = new SqlParameter("@DataId", SqlDbType.Int, 4);
            parameter.Value = DataId;
            TogoDelMoneyLogInfo model = null;
            using (SqlDataReader dr = SQLHelper.ExecuteReader(CommandType.Text, sql, parameter))
            {
                if (dr.Read())
                {
                    model = new TogoDelMoneyLogInfo();
        ...
        public int GetCount(string strWhere)
        {
            ...
            parameters[0].Value = "TogoDelMoneyLog";
        ...
        ///此代码由杭景科技代码内部生成器自动生成
        public IList<TogoDelMoneyLogInfo> GetList(int pagesize, int pageindex, string strWhere, string orderName, int orderType)
        {
            IList<TogoDelMoneyLogInfo> infos = new List<TogoDelMoneyLogInfo>();
            SqlParameter[] parameters = 
	         {
            ...
            parameters[0].Value = "TogoDelMoneyLog";
            parameters[1].Value = "DataId,UserId,DelMoney,AddDate,BuyItem,Inve1,Inve2,(select Name from points where unid=TogoDelMoneyLog.UserId) as TogoName";
        ...
        /// 此代码由杭景科技代码内部生成器自动生成
        /// <returns></returns>
        public int DelTogoDelMoneyLog(int Id)
        {
            StringBuilder str = new StringBuilder();
            str.Append("delete from TogoDelMoneyLog where DataId=@DataId");
            SqlParameter[] Para = 
	        {
        ...
        {
            StringBuilder str = new StringBuilder();
            str.Append("delete from TogoDelMoneyLog where DataId in ({0})");
