...
namespace Hangjing.SQLServerDAL
{
    ...
    /// 自动调度配置
    /// </summary>
    public partial class autodispatchconfig
    {

        /// <summary>
        /// 增加一条数据
        /// </summary>
        public int Add(autodispatchconfigInfo model)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("insert into autodispatchconfig(");
        ...
        /// 更新一条数据
        /// </summary>
        public int Update(autodispatchconfigInfo model)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update autodispatchconfig set ");
        ...
        /// 此代码由杭景科技代码内部生成器自动生成
        /// <param>dataid</param>
        /// <returns>autodispatchconfigInfo</returns>
        public autodispatchconfigInfo GetModel(int dataid)
{
	string sql = "select dataid,isopen,autotype,distance,reveint1,reveint2,revevar1,revevar2,revedate from autodispatchconfig where  dataid = @dataid";
	SqlParameter parameter = new SqlParameter("@dataid", SqlDbType.Int, 4);
	parameter.Value = dataid;
	autodispatchconfigInfo model = null;
	using (SqlDataReader dr = SQLHelper.ExecuteReader(CommandType.Text, sql, parameter))
	{
		if (dr.Read())
		{
			model = new autodispatchconfigInfo();
	...
	return model;
        ...
        /// <param>dataid</param>
        /// <returns>autodispatchconfigInfo</returns>
        public autodispatchconfigInfo GetModelByCity(int cityid)
        {
            string sql = "select dataid,isopen,autotype,distance,reveint1,reveint2,revevar1,revevar2,revedate from autodispatchconfig where  reveint2 = @cityid";
        ...
        {
            return (int)SQLHelper.ExecuteScalar(CommandType.StoredProcedure, "pagecount", new SqlParameter("@tblName", "autodispatchconfig"), new SqlParameter("@strWhere", strWhere));
        }
        ...
        ///此代码由杭景科技代码内部生成器自动生成
        public IList<autodispatchconfigInfo> GetList(int pagesize, int pageindex, string strWhere, string orderName, int orderType)
{
	IList<autodispatchconfigInfo> infos = new List<autodispatchconfigInfo>();
	SqlParameter[] parameters = 
			{
	...
	parameters[0].Value = "autodispatchconfig";
	...
	using (SqlDataReader dr = SQLHelper.ExecuteReader(CommandType.StoredProcedure, "pageselectpri", parameters))
	{
		while (dr.Read())
		{
			autodispatchconfigInfo info = new autodispatchconfigInfo();
	...
	return infos;
        ...
        public int Delautodispatchconfig(int Id)
        {
            ...
            str.Append("delete from autodispatchconfig where dataid=@dataid");
            SqlParameter[] Para = 
			{
        ...
        {
            StringBuilder str = new StringBuilder();
            str.Append("delete from autodispatchconfig where dataid in ({0})");
