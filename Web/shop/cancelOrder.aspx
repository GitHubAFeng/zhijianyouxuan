<%@ Page Language="C#" AutoEventWireup="true" CodeFile="cancelOrder.aspx.cs" Inherits="shop_cancelOrder" %>
...
<head runat="server">
    ...
    <title>退单-<%= SectionProxyData.GetSetValue(3)%></title>
            ...
            <div class="main">
                <div class="Precision_filter">
                    ...
                    <p class="field switch">
                        处理状态：
                        <input type="radio" id="have_1" name="field6" onclick="gourl('<%=getSortUrl("oe","1") %>    ')" />
                        <label for="radio0" class="cb-label"><span>待处理</span></label>
                        <input type="radio" id="have_2" name="field6" checked="checked" onclick="gourl('<%=getSortUrl("oe","2") %>    ')" />
                        <label for="radio1" class="cb-label selected"><span>已处理</span></label>
                    </p>
                </div>
                                ...
                                <div class="orderlist_box">
                                    <div class="order_info">
                                        <p class="timestate">
                                            ...
                                            <span class="<%# Convert.ToInt32(Eval("shopCancel")) == 0 ? "havenone": "havea"%>" style="line-height: 25px; margin-top: 3px; margin-right: 45px;">
                                                <%# WebUtility.ShopState(Eval("shopCancel").ToString())%>
                                                <i></i>
                                            </span>
                                    ...
                                    <p class="bottom_btn">
                                        <button style="<%# Convert.ToInt32(Eval("shopcancel"))==0 ? "": "display:none"%>" class="btn" onclick="acOrder(<%#Eval("OrderID") %>,'IsShopSet')" type="button">同意</button>
                                        <button style="<%# Convert.ToInt32(Eval("shopcancel"))==0 ? "": "display:none"%>" class="btn" onclick="opOrder(<%#Eval("OrderID") %>,'IsShopSet')" type="button">拒绝</button>
...
<script type="text/javascript">
    ...
    RefuseObject.prototype.SetState = function(state)
    {
        ...
        var para = "t=" + new Date().getTime() + "&id=" + id + "&method=setCancel&msg=" + escape(msg)+"&state="+state;
        showload_super();
        jQuery.ajax(
        {
            type: "post",
