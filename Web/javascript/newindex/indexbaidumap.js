...
function initialize() {
    ...
    //建立一个自动完成的对象
    ac = new BMap.Autocomplete({ "input": "tbindexsearchbox", "location": cityname + "市" });
...
function go_search() {
    ...
    if (key == "" || key == "请输入写字楼，小区，学校等地址信息") {
        //alert("请输入写字楼，小区，学校等地址信息");//由于添加了判断，这里不需要了
        return false;
    }
...
//选择城市
function go_city_search(cname, Lat, Lng) {
    $("#hfcityname").val(cname);
    $("#tbcityname").val(cname);
    $("#mapcityname").val(cname);
    $("#hidLat").val(Lat);
    $("#hidLng").val(Lng);
    initialize();
}