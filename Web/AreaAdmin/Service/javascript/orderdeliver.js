    ...
    jQuery.ajax(
        {
            ...
            success: function (msg) {
                ...
                winhtml += '<tr><td>车况：' + deli_latlng.carstate + '<img id=\"theimage\" border=\"0\" src=\"direction.jpg\" onload=\"rotateRight(\'theimage\',' + deli_latlng.direction + ');\" /></td><td>配送：' + deli_latlng.ordernum + '单</td></tr>'
                winhtml += '<tr><td>完成：' + deli_latlng.completeorder + '单</td><td>超时：' + deli_latlng.timeoutorder + '单</td></tr>'

               
                var myordercount = deli_latlng.orderlist.length;

                for (var xx = 0; xx < myordercount; xx++) {
                    winhtml += "<tr><td  colspan='2'>" + deli_latlng.orderlist[xx].UserName + "</span>(" + deli_latlng.orderlist[xx].Tel + ")(" + deli_latlng.orderlist[xx].Address + ")<span style='margin-left:10px;'>" + deli_latlng.orderlist[xx].TogoName + "</td></tr>";
                }
