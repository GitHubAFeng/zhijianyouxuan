...
public class ExcelHelper
{
    ...
    public static void DataGridViewToExcel(DataGridView myDgv, string saveFileName, bool isOpen,
        string saveFilePath, string strHeaderText, string[] titleNames)
    {
    ...
    /// <returns>内存流</returns>
    private static MemoryStream DataGridViewToExcel(DataGridView myDgv, string strHeaderText,
        string[] titleNames)
    {
        ...
        if (titleNames != null) //构建自定义列标题
        {
            if (titleNames.Length > 0)
            {
                for (int k = 0; k < titleNames.Length; k++)
                {
                    ...
                    cell.CellStyle = Getcellstyle(_hssfworkbook, stylexls.head);
                }
                rowIndex++;
            }
        }
